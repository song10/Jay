...
extension JSON {
    ...
    public subscript(key: String) -> JSON {
        get {
            ...
            let json:JSON = nil
            guard case .object(let dict) = self else { return json }
            if let val = dict[key] { return val } else {
                return json
            }
        }
        set(newValue) {
            // perform a suitable setting action here
            print("perform a suitable setting action here")
        }
    }
    ...
    public subscript(index: Int) -> JSON {
        get {
            // return an appropriate subscript value here
            let json:JSON = nil
            guard case .array(let array as [JSON?]) = self else { return json }
            if (index < array.count) {
                if let val = array[index] { return val }
            }
            return json
        }
        set(newValue) {
